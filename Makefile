...
FETCH_URL=https://ftp.gnu.org/gnu/gmp/gmp-5.1.3.tar.bz2
POD_NAME=gmp-5.1.3
...
fetch:
	...
	wget -O $(POD_NAME).tar.bz2 $(FETCH_URL)
	@touch fetched.touch

unarchive:
	@echo "\n UnArchiving $(POD_NAME) \n"
	@tar xjf $(POD_NAME).tar.bz2
	@touch unarchived.touch

build-source:
	@echo "\n Building $(POD_NAME) \n"
	@mkdir -p pod-build
	cd pod-build && ../$(POD_NAME)/configure --prefix=$(BUILD_PREFIX) --with-pic
	cd pod-build && make
	@touch built.touch
